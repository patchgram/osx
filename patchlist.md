...
| Show hidden gifts | `dylib` | Adds extra star gifts to the gift purchase menu, locally, by appending entries to the `payments.starGifts` response inside the runtime library. The injected gifts (price 50 Stars, not limited) appear at the **top** of the list; their stickers use the matching custom emoji resolved from `api.changes.tg`. This is a local display patch — buying a gift the server doesn't actually offer still fails server-side. No Telegram bytes are patched. |
