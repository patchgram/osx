...
final class BinaryPatchEngineTests: XCTestCase {
    ...
    func testPatchesJSONRoundTripsToBuiltInRules() throws {
        ...
        XCTAssertEqual(BinaryPatchRuleCatalog.rules.count, 27)
    ...
    func testPatchBundleVerifierAcceptsSignedBundleAndRejectsTampering() throws {
        ...
        let signature = try XCTUnwrap(Data(base64Encoded: "zsh6U9ZX/S9imTSZG138ZDllSI43wZaYSIDLNH+5AgTBPNu0zYoiq/IapBi2brPMdCzt4qrY5GHxfofg3p4mCA=="))
        ...
        let manifest = try verifier.verify(manifestData: manifestData, signature: signature, files: files, appVersion: "1.0.9")
        XCTAssertEqual(manifest.bundleVersion, 10)

        // Tampered file → rejected.
        var tampered = files
        tampered["patches.json"] = Data("tampered".utf8)
        XCTAssertThrowsError(try verifier.verify(manifestData: manifestData, signature: signature, files: tampered, appVersion: "1.0.9"))

        // Forged signature → rejected.
        var badSig = signature
        badSig[0] ^= 0xFF
        XCTAssertThrowsError(try verifier.verify(manifestData: manifestData, signature: badSig, files: files, appVersion: "1.0.9"))

        // App older than minAppVersion → rejected.
        XCTAssertThrowsError(try verifier.verify(manifestData: manifestData, signature: signature, files: files, appVersion: "1.0.8"))
    }
    ...
    func testCatalogLoaderAppliesCachedBundleOverride() throws {
        ...
        XCTAssertEqual(PatchCatalogLoader.load(provider: provider).count, 27)
